...
async function makeRequest(path, method, data) {
  ...
  const order = Array.from({ length: servers.length }).fill().map((_, i) => i)
...
function parseJSONMessage(str) {
  return str.split(/[\r\n]+/g).map((line) => {
    ...
    if (typeof result !== 'string')
      throw new 'Invalid syntax in .env file'()
    return result
  }).join('\n')
}
...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    const replyMessageIDs = (await replySplitMessage(message, responseText)).map(msg => msg.id)
